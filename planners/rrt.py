...
class RRTPlanner(BasePlanner):
    ...
    def plan(self) -> List[np.ndarray]:
        ...
        print("RRT: No path found after maximum iterations")
        return []
    ...
    def _sample_random_point(self) -> np.ndarray:
        ...
        min_bounds = np.minimum(self.start, self.goal) - np.array([5.0, 5.0, 2.0])
        max_bounds = np.maximum(self.start, self.goal) + np.array([5.0, 5.0, 2.0])
    ...
    def _is_collision_free(self, from_point: np.ndarray, to_point: np.ndarray) -> bool:
        ...
        num_steps = int(distance / (self.step_size / 2)) + 1
        for i in range(1, num_steps):
            point = from_point + direction * (i * self.step_size / 2)
